%{
LD_LIBRARY_PATH=/usr/local/cuda/lib64:local matlab -nodisplay
...
%{
lName = 'loop1_meanshift_S_is_XX';
netbasemodel.addLayer(lName, ...
    meanshift_S_is_XX(), ... softmaxlog logistic
    {sName}, lName);
sName = lName;

lName = 'loop1_meanshift_G_is_Gaussian';
netbasemodel.addLayer(lName, ...
    meanshift_G_is_Gaussian('delta', 0.2), ...
    {sName}, lName);
G_layer = lName;
sName = lName;

lName = 'loop1_meanshift_d_is_sumG';
netbasemodel.addLayer(lName, ...
    meanshift_d_is_sumG(), ...
    {sName}, lName);
sName = lName;

lName = 'loop1_meanshift_q_is_inv_d';
netbasemodel.addLayer(lName, ...
    meanshift_q_is_inv_d(), ...
    {sName}, lName);
sName = lName;

lName = 'loop1_meanshift_P_is_G_diag_q';
netbasemodel.addLayer(lName, ...
    meanshift_P_is_G_diag_q(), ...
    {G_layer, sName}, lName);
sName = lName;

lName = 'loop1_meanshift_Y_is_XP';
netbasemodel.addLayer(lName, ...
    meanshift_Y_is_XP(), ...
    {x_l2_norm_layer, sName}, lName);
sName = lName;


lName = 'loop1_meanshift_Y_l2norm';
netbasemodel.addLayer(lName, L2normalization(), sName, lName) ;
sName = lName;

lName = 'loop1_meanshift_cosSim';
gt_name =  sprintf('gt_ins');
netbasemodel.addLayer(lName, cosineSimilarity_randSample('randSampleRatio', 0.2), {sName, gt_name}, lName) ;
sName = lName;
...
% add regression loss
obj_name = sprintf('loop1_instSeg_reg');
netbasemodel.addLayer(obj_name, ...
    InstanceSegRegLoss_randSample('loss', 'cosinesimilarityabsregloss', 'lastLayerName', sName), ... softmaxlog logistic
    {sName, gt_name}, obj_name);

% add max-margin loss
obj_name = sprintf('loop1_instSeg_MM');
input_name = sName;
netbasemodel.addLayer(obj_name, ...
    InstanceSegMMLoss_randSample('loss', 'cosinesimilaritymmloss', 'marginAlpha_', 0.1, 'adaptiveMM', false, 'lastLayerName', sName), ...
    {input_name, gt_name}, obj_name)
