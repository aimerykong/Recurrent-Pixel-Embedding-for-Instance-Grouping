function [net,stats] = cnnTrain_augVOC2012_inst_div4(net, prefixStr, imdb, getBatch, varargin)
...
for epoch=start+1:opts.numEpochs    
    ...
    %     state.train = opts.train(randperm(numel(opts.train), 1000)) ; % shuffle
    state.val = opts.val(1:numel(opts.val)) ; % do not shuffle
    state.imdb = imdb ;

    if numel(opts.gpus) <= 1
        [stats.train(epoch),prof] = processEpoch_augVOC2012_inst_div4(net, state, opts.scalingFactor, opts, 'train', opts.numEpochs) ;
        stats.val(epoch) = processEpoch_augVOC2012_inst_div4(net, state, opts.scalingFactor, opts, 'val', opts.numEpochs) ;
        ...
        spmd
            ...
            [stats_.train, prof_] = processEpoch_augVOC2012_inst_div4(net_, state, opts.scalingFactor, opts, 'train') ;
            stats_.val = processEpoch_augVOC2012_inst_div4(net_, state, opts.scalingFactor, opts, 'val') ;
